...
//
// Returns the config portion of the stored {config, taskUid} wrapper (new format), or
// the raw stored value for old-format fingerprints (backward-compatible — entry IS the
// config directly). Returns null for the disabled state (buildEmbedderConfig returns null).
async function getAppliedConfig(plugin, indexName) {
	const stored = await settings.getOne(plugin.id, 'appliedEmbedders') || {};
	if (!Object.prototype.hasOwnProperty.call(stored, indexName)) return undefined;
	const entry = stored[indexName];
	if (entry === null) return null;  // disabled state
	if (typeof entry === 'object' && 'config' in entry && 'taskUid' in entry) {
		return entry.config;  // new format: {config, taskUid} wrapper
	}
	return entry;  // old format: entry IS the config directly
}
...
// Stores {config, taskUid} per index: taskUid is null for sync-verified success, the enqueued
// taskUid for optimistic-pending (timeout) state. Used by reattachPollers in lib/client.js to
// reconcile persisted tasks directly via getTask(uid) instead of querying the full task queue.
async function setAppliedConfigs(plugin, plans) {
	const stored = await settings.getOne(plugin.id, 'appliedEmbedders') || {};
	plans.forEach(({ indexName, config, taskUid }) => {
		stored[indexName] = { config, taskUid: taskUid ?? null };
	});
...
// Reads the persisted taskUid for one index (from the {config, taskUid} wrapper stored by
// setAppliedConfigs). Returns: a number (optimistic-pending task still running server-side),
// null (sync-verified success — no reconciliation needed), or undefined (old-format fingerprint
// without taskUid, or index never pushed). Used by reattachPollers in lib/client.js to
// reconcile persisted tasks directly via getTask(uid) instead of querying the full task queue.
async function getAppliedTaskUid(plugin, indexName) {
	const stored = await settings.getOne(plugin.id, 'appliedEmbedders') || {};
	if (!Object.prototype.hasOwnProperty.call(stored, indexName)) return undefined;
	const entry = stored[indexName];
	if (entry === null || entry === undefined) return undefined;
	if (typeof entry === 'object' && 'taskUid' in entry) {
		return entry.taskUid;
	}
	return undefined;  // old format — no taskUid stored
}
