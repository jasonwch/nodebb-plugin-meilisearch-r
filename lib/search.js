...
async function buildHybridOption(plugin) {
	if (!(await isSemanticSearchEnabled(plugin))) return undefined;
	return { embedder: EMBEDDER_NAME, semanticRatio: await getSemanticRatio(plugin) };
}
...
async function safeSearch(plugin, indexName, term, opts) {
	try {
		...
		if (opts.hybrid) {
			winston.error(`[plugin/meilisearch] Hybrid search on "${indexName}" failed (${reason}); falling back to keyword-only search`);
			plugin.notifyAdmins(`search:${indexName}`, {
				type: 'danger',
				titleKey: '[[meilisearch:admin.semanticSearchDegraded]]',
				message: `${indexName}: ${reason}`,
			});
			const { hybrid, ...keywordOnly } = opts;
			try {
				return await plugin.client.index(indexName).search(term, keywordOnly);
			} catch (fallbackErr) {
				winston.error(`[plugin/meilisearch] Keyword-only fallback search on "${indexName}" also failed: ${fallbackErr.message}`);
				return null;
			}
		}
		winston.error(`[plugin/meilisearch] Search on "${indexName}" failed: ${reason}`);
		plugin.notifyAdmins(`search:${indexName}`, {
			type: 'danger',
			titleKey: '[[meilisearch:admin.semanticSearchDegraded]]',
			message: `${indexName}: ${reason}`,
...
module.exports = function attachSearch(plugin) {
	...
	plugin.searchMessages = async function (data) {
		...
		const result = await safeSearch(plugin, 'chat_message', rawQuery, {
			...
			hybrid: await buildHybridOption(plugin),
		});
